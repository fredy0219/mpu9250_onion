  ...
  #  		time.sleep(0.1);
	...
	def mpu_self_test(self):
		...
		for i in xrange(200):
			...
			aAvg[0] += int(struct.unpack('>h',chr(raw_data[0])+chr(raw_data[1])))
			aAvg[1] += int(struct.unpack('>h',chr(raw_data[2])+chr(raw_data[3])))
			aAvg[2] += int(struct.unpack('>h',chr(raw_data[4])+chr(raw_data[6])))


			raw_data = i2c.readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6)
			gAvg[0] += int(struct.unpack('>h',chr(raw_data[0])+chr(raw_data[1])))
			gAvg[1] += int(struct.unpack('>h',chr(raw_data[2])+chr(raw_data[3])))
			gAvg[2] += int(struct.unpack('>h',chr(raw_data[4])+chr(raw_data[6])))
		...
		for i in xrange(200):
			raw_data = i2c.readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6)
			aSTAvg[0] += int(struct.unpack('>h',chr(raw_data[0])+chr(raw_data[1])))
			aSTAvg[1] += int(struct.unpack('>h',chr(raw_data[2])+chr(raw_data[3])))
			aSTAvg[2] += int(struct.unpack('>h',chr(raw_data[4])+chr(raw_data[6])))

			raw_data = i2c.readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6)
			gSTAvg[0] += int(struct.unpack('>h',chr(raw_data[0])+chr(raw_data[1])))
			gSTAvg[1] += int(struct.unpack('>h',chr(raw_data[2])+chr(raw_data[3])))
			gSTAvg[2] += int(struct.unpack('>h',chr(raw_data[4])+chr(raw_data[6])))
